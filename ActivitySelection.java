	...
	public List<Activity> selectActivitiesGreedy() {
		...
		List<Activity> acts = new ArrayList<Activity>();
		 if (!activities.isEmpty()) {
		    ...
		    // last Finish Time of last selected Activity
		    int lastFinishTime = acts.get(0).getFinish();

		    // Iterate through the rest of the activities
		    for (int i = 1; i < activities.size(); i++) {
		        Activity currentActivity = activities.get(i);
		        // Check if the current activity's start time is after the last selected activity's finish time
		        if (currentActivity.getStart() >= lastFinishTime) {
		            acts.add(currentActivity);
